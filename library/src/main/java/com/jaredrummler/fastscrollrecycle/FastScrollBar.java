...
package com.jaredrummler.fastscrollrecycle;
...
public class FastScrollBar {
  ...
  private boolean isSqlScan = false;
  ...
  public FastScrollBar(FastScrollRecyclerView rv, AttributeSet attrs) {
    ...
    isSqlScan = ta.getBoolean(R.styleable.FastScrollRecyclerView_isSqlScan,false);
    ta.recycle();
    recyclerView = rv;
  ...
  protected void draw(Canvas canvas) {
    ...
    if (trackPaint.getAlpha() > 0) {
      if(isSqlScan){
        canvas.drawRect(recyclerView.getMinimumWidth() - 28 + recyclerView.getOffset(), 0,
                        recyclerView.getMinimumWidth() - 28 + thumbWidth - thumbCurvature + recyclerView.getOffset(),
                        recyclerView.getHeight(), trackPaint);
      }else {
